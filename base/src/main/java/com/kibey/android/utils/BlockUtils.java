...
public class BlockUtils {
    ...
    private long mReceiveDispatchMessageTime;

    /**
     * 创建一个监控当前线程的BlockUtils对象，只能监控包含Looper对象的线程，所以当前线程必须包含Looper。
     */
    public BlockUtils() {
    ...
    private Printer createBlockPrinter() {
        return new Printer() {
             ...
             * 可以根据{@link Looper#loop()}中logging.println打印的Message格式，进行修正。
             */
            private void checkMessage(String s) {
                if (s.startsWith(">>>>> Dispatching to")) {
                    if (!mPrinterStart) {
                        Log.e(TAG, "错误：出现消息错乱：" + s);
                    }
                    mPrinterStart = true;
                }
     ...
     * @return 当前线程堆栈数据
     */
    public static String dumpStackWithTimeHead() {
        return dumpStackWithTimeHead(Thread.currentThread());
    }
    ...
    public static String dumpStackWithTimeHead(Thread dumpThread) {
        String head = "\n" + DateUtil.timestamp2Date18(System.currentTimeMillis() + "") + "时堆栈状态\n";
    ...
    private void receiveDispatchingMessage() {
        mReceiveDispatchMessageTime = System.currentTimeMillis();
    ...
    private void receiveFinishedMessage() {
        ...
        long delay = end - mReceiveDispatchMessageTime;
    ...
    public void install() {
        ...
        mLooper.setMessageLogging(createBlockPrinter());
    }
    ...
    private class PrintStaceInfoRunnable implements Runnable {
        ...
        private Thread mDumpThread = mLooper.getThread();
